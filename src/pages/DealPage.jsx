...
export default function DealPage() {
  ...
  function handleDealEdited(updatedDeal) {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/deals/${deal_id}/full`);
  ...
  async function handleRemoveDeal() {
    ...
    try {
      ...
      const res = await fetch(`https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/deals/${deal.deal_id}`, {
        method: "DELETE",
  ...
  async function handleUndoRemove() {
    ...
    try {
      ...
      if (!user) return;
      const token = await user.getIdToken();
      const res = await fetch(`https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/deals/${deal.deal_id}/reactivate`, {
        method: "PUT",
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
...
function VotingSection({ deal_id, initialVotes, userVote: initialUserVote }) {
  ...
  async function handleVote(vote_type) {
    ...
    try {
      ...
      if (!user) return;
      const token = await user.getIdToken();
      const res = await fetch(`https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/deals/addremove/vote`, {
        method: "PUT",
        headers: {
  ...
  React.useEffect(() => {
    async function fetchUserDealsCount() {
      if (deal.user_id && (!deal.user_total_deals || deal.user_total_deals === 0)) {
        try {
          const res = await fetch(`https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/deals/user/${deal.user_id}`);
          if (res.ok) {
...
function RelatedDealsSection({ deal }) {
  ...
  React.useEffect(() => {
    async function fetchRelated() {
      setLoading(true);
      try {
        const res = await fetch("https://capstone-deals-app-endpoints-git-main-kaiser-shahs-projects.vercel.app/categories-with-deals");
        if (res.ok) {
